...
export async function POST(request) {
  try {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0f0f0f; color: #fff; padding: 32px; border-radius: 12px;">
          ...
          <div style="text-align: center;">
            <a href="${approveUrl}" style="display: inline-block; background: #39ff14; color: #000; padding: 16px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px; letter-spacing: 2px;">
              ✅ APPROVE & SEND LINK
            </a>
          </div>
          ...
          <p style="color: #444; font-size: 11px; margin-top: 24px; text-align: center;">GameVault PH · Do not share this email</p>
        </div>
      `,
    });
    ...
    return Response.json({ error: error.message || 'Failed to send request' }, { status: 500 });
  }
}
